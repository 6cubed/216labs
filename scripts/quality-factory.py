...
def check_offline(
    ...
    for app in selected:
        ...
        try:
            ...
            elif app.app_id == "birdperch":
                # Bird Perch had a real-world regression where the happy-path upload
                # (MediaRecorder WebM/Opus) failed to decode. Probe that flow here
                # so CI catches it even if the ML model is unavailable.
                code, body = _birdperch_upload_probe(f"http://127.0.0.1:{host_port}/api/identify", timeout_s=timeout_s)
                body_l = (body or "").lower()
                if code == 400 and ("could not decode audio" in body_l or "format not recognised" in body_l):
                    detail = f"birdperch upload decode failed (status {code}): {body[:500]}"
                    reports[app.app_id] = CheckReport(status="fail", detail=detail)
                    errs.append(f"{app.app_id}: {detail}")
...
def _multipart_body(*, field_name: str, filename: str, content_type: str, payload: bytes) -> tuple[bytes, str]:
    boundary = "----216labs-boundary-7d4a3b2f8d1b"
    head = (
        f"--{boundary}\r\n"
...
def _birdperch_upload_probe(url: str, *, timeout_s: float) -> tuple[int | None, str]:
    """POST a tiny WebM/Opus clip; fail only on decode errors (model can be unavailable)."""
    # 0.25s sine, WebM/Opus (matches browser MediaRecorder happy path).
    clip = base64.b64decode(
        ""
    )
    ...
    req = urllib.request.Request(
        ...
        data=body,
        headers={
            "User-Agent": "quality-factory/1.0",
    ...
    try:
        ...
        # Try to surface server JSON even on HTTP error codes.
        if hasattr(e, "read"):
            try:
                raw = e.read(4000).decode("utf-8", errors="replace")
