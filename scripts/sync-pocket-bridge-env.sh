#!/usr/bin/env bash
# Pull TELEGRAM_BOT_TOKEN and TELEGRAM_OWNER_ID from the droplet admin SQLite DB into
...
set -euo pipefail
DB="$1"
sqlite3 "$DB" "SELECT key || '=' || value FROM env_vars WHERE key IN ('TELEGRAM_BOT_TOKEN','TELEGRAM_OWNER_ID') AND value IS NOT NULL AND trim(value) != '';"
REMOTE
mv "$OUT.tmp" "$OUT"
lines=$(wc -l <"$OUT" | tr -d ' ')
