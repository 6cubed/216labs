...
2. **Periodic sync (this doc)** — [`scripts/droplet-ghcr-sync.sh`](../scripts/droplet-ghcr-sync.sh) walks **running** Compose services whose image is `216labs/*` (or the GHCR form), pulls the matching GHCR tag, retags to `216labs/<service>:latest`, and runs `docker compose up -d --force-recreate` for that service. **Spine + revenue hot pool** (`caddy`, `activator`, `admin`, `landing`, `cron-runner`, `storybook`, `maxlearn`, `1pageresearch`, `kidgift`) plus **`zurichrunclubs`** (GHCR `:latest` lagged laptop deploys) are skipped by default (`SYNC_EXCLUDE_SERVICES`); use `SYNC_SERVICE=zurichrunclubs` to pull **only** that running service (other apps are left alone, even if they would normally sync). After sync the script regenerates the Caddyfile and **recreates Caddy only if that file changed** (or Caddy is down). A force-recreate every 20 minutes took the public edge down (`edge_proxy` / ECONNREFUSED :80). Then it runs [`droplet-ensure-spine.sh`](../scripts/droplet-ensure-spine.sh) (same Caddy rule). [`droplet-resource-pressure.sh`](../scripts/droplet-resource-pressure.sh) will not LRU-stop those protected services.
