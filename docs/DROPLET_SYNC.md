...
2. **Periodic sync (this doc)** — [`scripts/droplet-ghcr-sync.sh`](../scripts/droplet-ghcr-sync.sh) walks **running** Compose services whose image is `216labs/*` (or the GHCR form), pulls the matching GHCR tag, retags to `216labs/<service>:latest`, and runs `docker compose up -d --force-recreate` **only when the image ID changed** (same-digest pull must not bounce a live host). **Spine + revenue hot pool** (`caddy`, `activator`, `admin`, `landing`, `cron-runner`, `storybook`, `maxlearn`, `1pageresearch`, `kidgift`) plus human-visited **`zurichrunclubs`** and **`anchor-api`** are skipped by default (`SYNC_EXCLUDE_SERVICES`); use `SYNC_SERVICE=anchor-api` (or `zurichrunclubs`) to pull **only** that running service. After sync the script regenerates the Caddyfile and **recreates Caddy only if that file changed** (or Caddy is down). A force-recreate every 20 minutes took the public edge down (`edge_proxy` / ECONNREFUSED :80) and 302’d `anchor.6cubed.app`. Then it runs [`droplet-ensure-spine.sh`](../scripts/droplet-ensure-spine.sh) (same Caddy rule). [`droplet-resource-pressure.sh`](../scripts/droplet-resource-pressure.sh) will not LRU-stop those protected services.
...
- Same-digest `docker pull` is cheap (manifest check). Recreate only when the running container’s image ID differs from `216labs/<svc>:latest` after pull — a same-digest `--force-recreate` 302s human-visited hosts.
...
- **`DEPLOY_IMAGE_SOURCE=local`:** `deploy.sh` now transfers when the **image ID** for a tag differs on the server (not only when the tag is missing), so local builds are not skipped after `DEPLOY_SKIP_BUILD=1`. Images copy as a **gzip tar then `docker load`** (not `docker save | ssh | docker load`) — a pipe through sshd OOMs the 1GB droplet and looks like `connection refused` for a minute.
