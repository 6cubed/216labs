...
| **Showroom / hotswap** | `DEPLOY_SHOWROOM=1` + `DEPLOY_RUNTIME_APPS` (or default `admin landing`) | Keeps a **small hot pool** running and pulls only those images from GHCR; other apps **cold-start via the activator**: Caddy redirects to `/warmup`, activator LRU-evicts non-protected containers if `ACTIVATOR_MAX_CONCURRENT_APPS` is exceeded, pulls `ghcr.io/…/<app>:latest` (retag to `216labs/<app>:latest`), then `compose up`. **GHCR credentials** come from `GHCR_TOKEN` / `GHCR_USERNAME` in env **or** from admin `env_vars` in `216labs.db` (same keys as `droplet-ghcr-sync.sh`), so pulls work without copying PATs into `.env` when they only live in the DB. Set `ACTIVATOR_REMOVE_IMAGE_ON_EVICT` to free disk after eviction. |
