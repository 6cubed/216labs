...
|--------|-------------------|----------------|
| **Which apps deploy** | Admin DB `deploy_enabled` + optional cap | DB stays source of truth; cap via `DEPLOY_MAX_APPS` (default **11**; override in env) |
| **Showroom / hotswap** | `DEPLOY_SHOWROOM=1` + `DEPLOY_RUNTIME_APPS` (or default `admin landing`) | **Goal:** browse **100+ apps** from one droplet with only a **small hot pool** of containers. Deploy keeps spine + chosen apps warm; everything else **cold-starts via the activator**. Caddy redirects to `/warmup`; the activator **aggressively LRU-evicts** non-protected containers when `ACTIVATOR_MAX_CONCURRENT_APPS` (default **6**) is exceeded, **pulls** `ghcr.io/…/<app>:latest` if needed (retag to `216labs/<app>:latest`), then `compose up` the **requested** app. **Public** GHCR images pull **anonymously**. **`GHCR_TOKEN` / `GHCR_USERNAME`** (from env or admin `env_vars` in `216labs.db`) are only needed for **private** packages. **`ACTIVATOR_REMOVE_IMAGE_ON_EVICT`** defaults **true** in compose to free disk after eviction (next cold start may pull from GHCR). |
| **Subset pull / build (not showroom)** | `DEPLOY_RUNTIME_APPS` without `DEPLOY_SHOWROOM=1` | Limits **GHCR pull** or **local build+transfer** to those app IDs only (faster; avoids unrelated private-image failures). **`docker compose up` still targets the full enabled catalogue** — other running containers are **not** stopped. Apps **outside the 11-app catalogue cap** still work when listed here if `docker-compose.yml` defines the service (`deploy.sh` prints a NOTE and pull/recreates anyway). Use showroom mode when you intentionally want a minimal running pool. |
