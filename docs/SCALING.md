...
| **Subset pull / build (not showroom)** | `DEPLOY_RUNTIME_APPS` without `DEPLOY_SHOWROOM=1` | Limits **GHCR pull** or **local build+transfer** to those app IDs only (faster; avoids unrelated private-image failures). **`docker compose up` still targets the full enabled catalogue** — other running containers are **not** stopped. Use showroom mode when you intentionally want a minimal running pool. |
