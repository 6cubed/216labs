...
import { X, Mail, Lock, User, ArrowRight, Shirt, Globe, Bell } from "lucide-react";
...
export default function AuthModal({ open, onClose, onSuccess }: AuthModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        await signup(name, email, password, emailOptIn, country);
      } else {
          ...
          >
            ...
            <div className="px-8 pt-8 pb-2 text-center">
              ...
              <h2 className="font-display text-2xl font-semibold text-surface-900">
                {mode === "signup" ? "Create your style profile" : "Welcome back"}
              </h2>
              <p className="mt-1.5 text-sm text-surface-800/50">
                {mode === "signup"
                  ? "Sign up to save preferences and get your daily look"
            ...
            <form onSubmit={handleSubmit} className="space-y-3 px-8 pt-4 pb-8">
              ...
              {mode === "signup" && (
                <>
                  <div className="relative">
                    <Globe className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-surface-800/30" />
                    <select
                      value={country}
                      onChange={(e) => setCountry(e.target.value)}
                      className="w-full rounded-xl border border-surface-200 bg-surface-50/50 py-3 pl-10 pr-4 text-sm text-surface-900 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-200/50 appearance-none"
                    >
                      {COUNTRIES.map((c) => (
                  ...
                  <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-surface-200 bg-surface-50/30 p-3.5 transition-colors hover:bg-brand-50/30">
                    <div className="relative mt-0.5 flex-shrink-0">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`h-4.5 w-4.5 rounded-md border-2 flex items-center justify-center transition-all ${
                        emailOptIn
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <Bell className="h-3.5 w-3.5 text-brand-500 flex-shrink-0" />
                        <span className="text-sm font-medium text-surface-900">Daily outfit emails</span>
                      </div>
                      <p className="mt-0.5 text-xs text-surface-800/50 leading-relaxed">
                        Receive 8 personalised outfit looks in your inbox every morning — tailored to your style with local shopping links.
                      </p>
                    </div>
                  </label>
                </>
