...
export function EnvVarEditor({ vars }: { vars: EnvVarRow[] }) {
  ...
  const grouped = useMemo(() => {
    return {
      ...
      shared: vars.filter(
        (v) =>
          !v.key.startsWith("AGIMEMES_") &&
          !v.key.startsWith("PIPESECURE_") &&
          !v.key.startsWith("PRIORS_") &&
          !v.key.startsWith("AGITSHIRTS_")
      ),
    };
  }, [vars]);
                ...
                <button
                  ...
                  onClick={() =>
                    startTransition(async () => {
                      ...
                      setSavedKey(row.key);
                      setTimeout(() => setSavedKey((s) => (s === row.key ? null : s)), 1500);
                    })
                  }
  ...
  return (
    <section className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {grouped.shared.length > 0 && renderGroup("Shared", grouped.shared)}
      </div>
    </section>
  );
}
