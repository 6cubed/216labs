...
3. **Cold-start pull** — On a **cold** service (not already running and healthy), the activator calls **`docker pull`** from GHCR (`ACTIVATOR_REGISTRY_PREFIX/<short>:latest`, short name from the compose **image** line) **before** the first `compose up`, so local stale `216labs/<short>:latest` tags are refreshed from CI. Set `ACTIVATOR_PULL_BEFORE_COLD_START=false` to disable (emergency only). For **`ramblingradio`** and **`stroll`** (override with `ACTIVATOR_ERRORS_RUNTIME_SERVICES`), a pulled image must contain `node_modules/@216labs/errors/dist/express.cjs`; otherwise the activator **keeps** a working local tag instead of retagging broken GHCR `:latest`. If `compose up` still fails, it retries pull + optional `ACTIVATOR_TRY_DOCKER_PULL` (Docker Hub). **Public** GHCR packages are pulled **anonymously** (no token). **`GHCR_TOKEN`** (read:packages) + **`GHCR_USERNAME`** are required when the package is **private** — typical for **org-scoped** CI images unless you explicitly publish as public. If cold start fails with “no such image” and `GET /healthz` shows `ghcr_auth_configured: false`, configure a PAT (or run `./deploy.sh` to transfer images). Images are published by `.github/workflows/ghcr-publish.yml`.
